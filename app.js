...
function annualEmploymentTax(grossAnnual){
  const pension=grossAnnual*.20;
  const taxable=Math.max(0,grossAnnual-pension-allowance()*12);
  const lowerBase=Math.min(taxable,60000);
  const higherBase=Math.max(0,taxable-60000);
  let tax=lowerBase*N('taxLow')/100+higherBase*N('taxHigh')/100;

  // Annual youth relief applies only to the proportional tax on the lower-rate base
  // attributable to employment income. In this MVP all taxable income modeled here
  // is employment income, so we apply the selected relief to the lower-rate tax.
  const youth=N('youthRelief');
  const lowerTax=lowerBase*N('taxLow')/100;
  tax-=lowerTax*youth;

  return {pension,taxable,lowerBase,higherBase,tax,net:grossAnnual-pension-tax};
}
...
function calc(j){
  const grossAnnual=j.salary*j.months+j.bonus;
  const tax=annualEmploymentTax(grossAnnual);

  // Split annual employment net between regular salary and bonus proportionally
  // for display only. Total net is calculated annually, which is the important figure.
  const regularGross=j.salary*j.months;
  const regularShare=grossAnnual?regularGross/grossAnnual:0;
  const ns=tax.net*regularShare;
  const nb=tax.net-ns;

  const direct=j.other+j.regres+j.boz+j.usk+j.meal*12+j.travelPay*12;

  const workableDays=Math.max(0,260-j.vacation-N('weekdayHolidays'));
  const officeShare=Math.max(0,Math.min(1,(5-j.remote)/5));
  const office=workableDays*officeShare;
  const hours=office*j.commute*2/60;
  const cost=office*j.travelCost;

  const cash=tax.net+direct;
  const financial=cash+j.benefits-cost;
  const total=financial-hours*N('hourValue');

  const sm=Math.min(100,cash/520);
  const sf=Math.min(100,j.remote/5*100);
  const st=Math.max(0,100-hours/3);
  const sv=Math.min(100,j.vacation/30*100);
  const score=Math.round(sm*.45+sf*.2+st*.2+sv*.15);
...
function calculate(){
  ...
  <tr><td>Procijenjeni neto dio bonusa</td><td>${fmt(c.nb)}</td><td>${fmt(nn.nb)}</td></tr><tr><td>Godišnji porez na dohodak</td><td>${fmt(c.tax.tax)}</td><td>${fmt(nn.tax.tax)}</td></tr>
...
function saveDraft(){
  ...
  localStorage.setItem('worthswitchDraftV51',JSON.stringify(data));
}
function restoreDraft(){
  const data=JSON.parse(localStorage.getItem('worthswitchDraftV51')||'{}');
...
function saveComparison(){
  if(!last.c)return;
  let a=JSON.parse(localStorage.getItem('worthswitchSavedV51')||'[]');
  ...
  localStorage.setItem('worthswitchSavedV51',JSON.stringify(a.slice(0,20)));
...
function deleteSaved(id){let a=JSON.parse(localStorage.getItem('worthswitchSavedV51')||'[]').filter(x=>x.id!==id);localStorage.setItem('worthswitchSavedV51',JSON.stringify(a));renderSaved()}
